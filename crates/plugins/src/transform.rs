...
use photopipeline_core::{
    ...
    PixelBuffer, PixelFormat, ColorSpace, GpuBackend, ChannelLayout,
                ...
                fields: vec![
                    ...
                    ParameterField {
                        ...
                        field_type: ParameterType::Float {
                            min: 1.0, max: 1000.0, step: 1.0, precision: 1,
                            unit: Some("%".into()), logarithmic: false,
                            style: Default::default(),
                        },
            ...
            ParameterSection {
                ...
                fields: vec![
                    ParameterField {
                        ...
                        help_url: None,
                        field_type: ParameterType::Float {
                            min: -360.0, max: 360.0, step: 0.1, precision: 1,
            ...
            ParameterSection {
                ...
                fields: vec![
                    ParameterField {
                        id: "filter_type".into(),
                        label: "Filter".into(),
                        description: Some("Resampling filter (Lanczos3 via Halide, bilinear for now)".into()),
                        help_url: None,
                        field_type: ParameterType::Enum {
                            options: vec![
                                EnumOption {
                                    value: "bilinear".into(), label: "Bilinear".into(),
                                    description: Some("Fast, good for upscaling".into()),
                                    icon: None, tags: vec![], recommended: false,
                                },
                                EnumOption {
                                    value: "lanczos3".into(), label: "Lanczos3".into(),
                                    description: Some("High quality, will use Halide".into()),
                                    icon: None, tags: vec![], recommended: true,
                                },
                                EnumOption {
                                    value: "nearest".into(), label: "Nearest Neighbor".into(),
...
static GUI_SCHEMA: LazyLock<GuiSchema> = LazyLock::new(|| {
    GuiSchema {
        layout: GuiLayout::Standard {
            sections: vec![
                GuiSection { param_section_id: "resize".into(), title_visible: true, style: SectionStyle::Card },
                GuiSection { param_section_id: "rotation".into(), title_visible: true, style: SectionStyle::Card },
                GuiSection { param_section_id: "crop".into(), title_visible: true, style: SectionStyle::Card },
                GuiSection { param_section_id: "filter".into(), title_visible: true, style: SectionStyle::CollapsibleCard },
            ],
        },
        icon: Some("maximize".into()),
...
impl TransformPlugin {
    pub fn new() -> Self {
        Self { id: "photopipeline.plugins.transform".to_string() }
    }
}
...
impl Plugin for TransformPlugin {
    ...
    fn category(&self) -> PluginCategory { PluginCategory::Transform }
    fn description(&self) -> &str { "Resize, rotate, and crop images with configurable filters" }
    fn tags(&self) -> &[String] { &TAGS }
    fn requires_pixel_access(&self) -> bool { true }
    fn produces_pixel_output(&self) -> bool { true }
    fn supported_hardware(&self) -> HardwareRequirement { HardwareRequirement { min_ram_mb: 256, ..Default::default() } }
    ...
    async fn validate(&self, params: &ParameterSet) -> PluginResult<Vec<ValidationIssue>> {
        ...
        let crop = params.get("crop_enabled").and_then(|v| v.as_bool()).unwrap_or(false);
        if crop {
...
impl PixelProcessor for TransformPlugin {
    ...
    fn supported_color_spaces(&self) -> Vec<ColorSpace> {
        vec![ColorSpace::SRGB, ColorSpace::LINEAR_SRGB, ColorSpace::DISPLAY_P3, ColorSpace::ADOBE_RGB]
    }
    ...
    async fn process_pixels(
        ...
        let mode = params.get_str("resize_mode").unwrap_or("none");
        let crop_enabled = params.get("crop_enabled").and_then(|v| v.as_bool()).unwrap_or(false);
        let angle = params.get("angle").and_then(|v| v.as_f64()).unwrap_or(0.0);
        let flip_h = params.get("flip_horizontal").and_then(|v| v.as_bool()).unwrap_or(false);
        let flip_v = params.get("flip_vertical").and_then(|v| v.as_bool()).unwrap_or(false);
        ...
        match mode {
            ...
            "long_edge" => {
                ...
                if input.width >= input.height {
                    target_w = long;
                    target_h = (input.height as f64 * long as f64 / input.width as f64).round() as u32;
                } else {
                    target_h = long;
                    target_w = (input.width as f64 * long as f64 / input.height as f64).round() as u32;
                }
                target_w = target_w.max(1);
                target_h = target_h.max(1);
            }
            "short_edge" => {
                ...
                if input.width <= input.height {
                    target_w = short;
                    target_h = (input.height as f64 * short as f64 / input.width as f64).round() as u32;
                } else {
                    target_h = short;
                    target_w = (input.width as f64 * short as f64 / input.height as f64).round() as u32;
                }
                target_w = target_w.max(1);
                target_h = target_h.max(1);
            }
            "megapixels" => {
                ...
                let target_pixels = (mp * 1_000_000.0) as f64;
        ...
        match filter {
            ...
            "lanczos3" => {
                bilinear_resize(input, output, target_w, target_h, channels, in_stride, out_stride);
            }
            _ => {
                bilinear_resize(input, output, target_w, target_h, channels, in_stride, out_stride);
            }
        ...
        if flip_h || flip_v {
            flip_buffer(output, target_w, target_h, channels, out_stride, flip_h, flip_v);
        }
...
fn nearest_resize(
    input: &PixelBuffer, output: &mut PixelBuffer,
...
static TAGS: LazyLock<Vec<String>> = LazyLock::new(|| {
    vec![
        "transform".into(), "resize".into(), "rotate".into(), "crop".into(),
        "scale".into(), "geometry".into(), "bilinear".into(), "lanczos".into(),
    ]
});
