                ...
                fields: vec![
                    ...
                    ParameterField {
                        ...
                        field_type: ParameterType::Enum {
                            options: vec![
                                EnumOption {
                                    value: "u8".into(), label: "8-bit".into(),
                                    description: None,
                                    icon: None, tags: vec![], recommended: false,
                                },
                                EnumOption {
...
impl TiffEncoderPlugin {
    pub fn new() -> Self {
        Self { id: "photopipeline.plugins.tiff_encoder".to_string() }
    }
}
...
impl Plugin for TiffEncoderPlugin {
    ...
    fn name(&self) -> &str { "TIFF Encoder" }
    fn version(&self) -> PluginVersion { PluginVersion::new(1, 0, 0) }
    fn category(&self) -> PluginCategory { PluginCategory::Format }
    fn description(&self) -> &str { "Encode images as TIFF files with configurable compression" }
...
impl FormatProcessor for TiffEncoderPlugin {
    ...
    async fn decode(&self, _data: &[u8], _options: &DecodeOptions) -> PluginResult<DecodedImage> {
        Err(PluginError::UnsupportedFormat("TIFF decoding not supported by encoder plugin".into()))
    }
    ...
    async fn encode(
        ...
        let strip_byte_count = (width as u32 * samples_per_pixel as u32 * bytes_per_sample) as u32;
        ...
        let total_size = strip_data_start + (strip_byte_count as u32 * strip_count) as u32;
        ...
        write_ifd_long(&mut buf, 278, &[height as u32]);
        ...
        for i in 0..strip_count {
            let offset = strip_data_start + i * strip_byte_count as u32;
            buf.extend_from_slice(&offset.to_le_bytes());
        }

        let pixel_data = &image.data.data;
        let data_size = width as usize * height as usize * samples_per_pixel as usize * bytes_per_sample as usize;
        let copy_len = data_size.min(pixel_data.len());
        buf.resize(strip_data_start as usize + copy_len, 0);
        buf[strip_data_start as usize..strip_data_start as usize + copy_len].copy_from_slice(&pixel_data[..copy_len]);
...
static TAGS: LazyLock<Vec<String>> = LazyLock::new(|| {
    vec![
        "format".into(), "tiff".into(), "encode".into(), "output".into(),
        "lossless".into(), "16bit".into(),
    ]
});
