...
use photopipeline_core::{
    ...
    PixelBuffer, PixelFormat, ColorSpace, GpuBackend, AiBackend,
    ValidationIssue, HardwareRequirement, ProcessingStats,
    Tensor,
};
use photopipeline_plugin::{
    Plugin, PixelProcessor, AiProcessor, ProgressSink,
    PluginConfig,
                ...
                fields: vec![
                    ParameterField {
                        ...
                        field_type: ParameterType::Enum {
                            options: vec![
                                EnumOption {
                                    value: "lightweight_v1".into(), label: "Lightweight v1".into(),
                                    description: Some("Fast, low memory. Good for ISO 100-3200".into()),
                                    icon: None, tags: vec!["fast".into()], recommended: true,
                                },
                                EnumOption {
                                    value: "standard_v2".into(), label: "Standard v2".into(),
                                    description: Some("Balanced quality/speed. ISO 100-12800".into()),
                                    icon: None, tags: vec!["balanced".into()], recommended: false,
                                },
                                EnumOption {
                                    value: "high_quality_v2".into(), label: "High Quality v2".into(),
                                    description: Some("Maximum quality. ISO 100-51200".into()),
                                    icon: None, tags: vec!["quality".into(), "slow".into()], recommended: false,
                                },
                                EnumOption {
                                    value: "raw_denoise_v1".into(), label: "RAW Denoise v1".into(),
                                    description: Some("Operates on RAW data before demosaicing".into()),
                                    icon: None, tags: vec!["raw".into()], recommended: false,
                                },
                            ],
                ...
                fields: vec![
                    ParameterField {
                        ...
                        field_type: ParameterType::Enum {
                            options: vec![
                                EnumOption {
                                    value: "onnx_cpu".into(), label: "ONNX (CPU)".into(),
                                    description: Some("ONNX Runtime on CPU. Works everywhere.".into()),
                                    icon: None, tags: vec!["cpu".into()], recommended: true,
                                },
                                EnumOption {
                                    value: "onnx_cuda".into(), label: "ONNX (CUDA)".into(),
                                    description: Some("ONNX Runtime with CUDA GPU acceleration".into()),
                                    icon: None, tags: vec!["gpu".into(), "cuda".into()], recommended: false,
                                },
                                EnumOption {
                                    value: "tensorrt".into(), label: "TensorRT".into(),
                                    description: Some("NVIDIA TensorRT optimized inference".into()),
                                    icon: None, tags: vec!["gpu".into(), "nvidia".into()], recommended: false,
                                },
                                EnumOption {
                                    value: "coreml".into(), label: "CoreML (ANE)".into(),
                                    description: Some("Apple Neural Engine via CoreML".into()),
                                    icon: None, tags: vec!["apple".into()], recommended: false,
                                },
                                EnumOption {
                                    value: "openvino".into(), label: "OpenVINO".into(),
                                    description: Some("Intel OpenVINO with GPU/NPU support".into()),
                                    icon: None, tags: vec!["intel".into()], recommended: false,
                                },
                            ],
                            display: Default::default(),
                        },
                    ...
                    ParameterField {
                        id: "use_fp16".into(),
                        label: "FP16".into(),
                        description: Some("Use half-precision (FP16) for faster inference on GPU".into()),
                        help_url: None,
...
static GUI_SCHEMA: LazyLock<GuiSchema> = LazyLock::new(|| {
    GuiSchema {
        layout: GuiLayout::Standard {
            sections: vec![
                GuiSection { param_section_id: "model".into(), title_visible: true, style: SectionStyle::Card },
                GuiSection { param_section_id: "strength".into(), title_visible: true, style: SectionStyle::Card },
                GuiSection { param_section_id: "hardware".into(), title_visible: true, style: SectionStyle::CollapsibleCard },
            ],
        },
        ...
        aux_views: vec![AuxView::Histogram, AuxView::ProgressBar, AuxView::StatusText],
        min_panel_width: 360,
    }
});

static STANDARD_MODEL_INFO: LazyLock<ModelInfo> = LazyLock::new(|| {
    ModelInfo {
...
impl Plugin for AiDenoisePlugin {
    ...
    fn description(&self) -> &str { "AI-powered image denoising using ONNX Runtime" }
    ...
    async fn validate(&self, params: &ParameterSet) -> PluginResult<Vec<ValidationIssue>> {
        ...
        let strength = params.get("denoise_strength").and_then(|v| v.as_f64()).unwrap_or(50.0);
        ...
        let detail = params.get("detail_preservation").and_then(|v| v.as_f64()).unwrap_or(50.0);
        if detail < 0.0 || detail > 100.0 {
            issues.push(ValidationIssue::Error {
...
impl PixelProcessor for AiDenoisePlugin {
    ...
    async fn process_pixels(
        ...
        let strength = params.get("denoise_strength").and_then(|v| v.as_f64()).unwrap_or(50.0);
        let _detail = params.get("detail_preservation").and_then(|v| v.as_f64()).unwrap_or(50.0);
        let _color = params.get("color_noise_reduction").and_then(|v| v.as_f64()).unwrap_or(75.0);
        ...
        if strength < 1.0 {
            ...
            return Ok(ProcessingStats {
                elapsed_ms: 0, cpu_time_ms: 0, gpu_time_ms: None,
                peak_memory_mb: input.data.data.len() as u64 / (1024 * 1024),
                input_pixels: input.pixel_count(), output_pixels: input.pixel_count(),
            });
        }
        ...
        Ok(ProcessingStats {
            elapsed_ms: 0,
            cpu_time_ms: 0,
            gpu_time_ms: if self.current_backend.read().is_some() { Some(0) } else { None },
...
impl AiProcessor for AiDenoisePlugin {
    ...
    async fn load_model(&mut self, backend: &AiBackend) -> PluginResult<()> {
        ...
        *self.current_backend.write() = Some(backend.clone());
    ...
    async fn unload_model(&mut self) -> PluginResult<()> {
        if *self.model_loaded.read() {
            tracing::info!("AI Denoise: unloading model '{}'", self.current_model.read());
        }
    ...
    async fn infer(&self, input: &Tensor, params: &ParameterSet) -> PluginResult<Tensor> {
        let _strength = params.get("denoise_strength").and_then(|v| v.as_f64()).unwrap_or(50.0);
...
static TAGS: LazyLock<Vec<String>> = LazyLock::new(|| {
    vec![
        "ai".into(), "denoise".into(), "onnx".into(), "machine-learning".into(),
        "gpu".into(), "cuda".into(), "tensorrt".into(), "coreml".into(),
        "enhance".into(), "neural".into(),
    ]
});
