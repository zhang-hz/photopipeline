...
use photopipeline_engine::{PipelineTemplate, TemplateNode, TemplateEdge};
use photopipeline_plugin::registry::Registry;
use photopipeline_plugin::ProgressSink;
use photopipeline_plugins;
use test_harness::fixtures::image::{known_value_solid_u8, color_bars};
use test_harness::fixtures::metadata::*;
use std::sync::Arc;
use uuid::Uuid;

fn make_registry() -> Arc<Registry> { let r = Arc::new(Registry::new()); photopipeline_plugins::register_all(&r); r }
fn make_image_info() -> ImageInfo { ImageInfo { id: Uuid::new_v4(), path: "/tmp/test_meta.ppm".into(), filename: "test_meta.ppm".into(), format: ImageFormat::PPM, width: 64, height: 64, file_size_bytes: 5000, pixel_format: PixelFormat::U8, color_space: ColorSpace::SRGB } }
fn rt() -> tokio::runtime::Runtime { tokio::runtime::Runtime::new().unwrap() }
fn make_executor(reg: Arc<Registry>) -> photopipeline_engine::NodeExecutor { let resolver = Arc::new(photopipeline_engine::ParameterResolver::new()); photopipeline_engine::NodeExecutor::new(reg, resolver) }
...
fn metadata_exif_sony_write_read_roundtrip() {
    ...
    let result = rt().block_on(async { executor.execute(&graph, &info, Some(pb), &metadata, Box::new(NoopProgress)).await });
    assert!(result.is_ok(), "pipeline failed: {:?}", result.err());
    let exec_result = result.unwrap();
    let exif = exec_result.metadata.exif.as_ref().expect("exif should be present");
...
fn metadata_gps_beijing() {
    ...
    assert!(result.is_ok(), "pipeline failed: {:?}", result.err());
    let exec_result = result.unwrap();
    let gps = exec_result.metadata.gps.as_ref().expect("gps should be present");
...
fn metadata_full_all_sections() {
    ...
    let xmp = exec_result.metadata.xmp.as_ref().expect("xmp should be present");
    assert_eq!(xmp.title.as_deref(), Some("Test Image"));
    assert_eq!(xmp.creator.as_deref(), Some("Test Author"));

    let iptc = exec_result.metadata.iptc.as_ref().expect("iptc should be present");
...
fn metadata_empty_no_corruption() {
    ...
    assert!(result.is_ok(), "pipeline failed: {:?}", result.err());
    let exec_result = result.unwrap();
    let buf = exec_result.buffer.as_ref().expect("output buffer should exist");
